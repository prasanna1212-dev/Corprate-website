...
export default function DomeGallery({
  ...
  grayscale = true,
  className = "",
  ...restProps // Capture any remaining props
}) {
...
useGesture(
  ...
  },
  {
    ...
    // ✅ THIS is the key: taps won't be treated as drags
    drag: {
      filterTaps: true,
      threshold: 10, // keeps your existing drag feel
    },
  }
);
  ...
  const onTileClick = useCallback(
    (e) => {
      console.log("🔥 TILE CLICKED!", e.currentTarget);
      e.preventDefault();
      e.stopPropagation();
      if (draggingRef.current) {
        console.log("❌ Blocked: dragging");
        return;
      }
      if (movedRef.current) {
        console.log("❌ Blocked: moved");
        return;
      }
      // ✅ REDUCED timing from 100ms to 20ms
      if (performance.now() - lastDragEndAt.current < 20) {
        console.log("❌ Blocked: too soon after drag");
        return;
      }
      if (openingRef.current) {
        console.log("❌ Blocked: already opening");
        return;
      }
  ...
  // Clean props for DOM element - only pass valid HTML attributes
  const validDomProps = {
    className: `sphere-root ${className}`.trim(),
    style: {
      ["--segments-x"]: segments,
      ["--segments-y"]: segments,
  ...
  onClick={(e) => {
    // ✅ Works because filterTaps prevents drag from consuming taps
    if (openingRef.current) return;
    if (focusedElRef.current) return;

    // extra safety: don't open if it was a drag
    if (movedRef.current) return;
    if (performance.now() - lastDragEndAt.current < 60) return;
