...
const SparkleNavbar = ({ items, color = "#00fffc", onItemClick, activeIndex: externalActiveIndex }) => {
  ...
  // Sync external activeIndex changes (scroll-based) - but don't animate during clicks
  useEffect(() => {
    if (externalActiveIndex !== undefined && externalActiveIndex !== activeIndex && !isAnimating) {
      setActiveIndex(externalActiveIndex);
    }
  }, [externalActiveIndex, activeIndex, isAnimating]);
  ...
  // Handle both initial setup and external activeIndex changes (scroll-based)
  useLayoutEffect(() => {
    const activeButton = buttonRefs.current[activeIndex];
    if (navRef.current && activeElementRef.current && activeButton && !isAnimating) {
      // Simple position update for external changes (no sparkle animation)
  ...
  // Click handler with full sparkle animation (like reference)
  const handleClick = (index) => {
    ...
    // Block external updates during animation
    setIsAnimating(true);
      ...
      keyframes: [
        ...
        },
        {
          ...
          onComplete: () => {
            ...
            setIsAnimating(false); // Allow external updates again
          },
        },
      ],
    });
  ...
  return (
    <>
      <style>{`
        ...
        .sparkle-navigation-menu ul li button {
        ...
        .sparkle-navigation-menu ul li:not(.active):hover button {
